...
const quizData = [
  ...
  },
  {
    ...
    options: [`Yes — because what goes in must come out (the net flux through the boundary must be zero)`, `Only if all Dirichlet values are also specified simultaneously`, `No, the 2D Neumann problem never has a solution`, `Only if boundary values are identically zero everywhere`],
    answer: 0,
    answerText: ``,
    intuition: ``,
  ...
  },
  {
    ...
    options: [`March forward in time: at each step update u by adding timestep times Laplacian(u)`, `Solve it as an elliptic (Laplace) equation ignoring time`, `Solve analytically using Fourier transforms only`, `Apply boundary conditions only at the final timestep`],
    answer: 0,
    answerText: ``,
    intuition: ``,
  ...
  },
  {
    ...
    options: [`Track height u and velocity v at each grid point, update both each timestep using second-order time integration`, `Compute the Fourier transform of the initial conditions and propagate analytically`, `Use only Dirichlet boundary conditions and solve once`, `Treat it as a parabolic equation with only one time derivative`],
    answer: 0,
    answerText: ``,
    intuition: ``,
  ...
  },
  {
    ...
    options: [`Implement algorithms for one specific geometry type only and do not generalize`, `Always start with discrete data structures (grids or graphs) and optimize later`, `Use machine learning to learn algorithms from example data rather than deriving them`, `Formulate problems as PDEs on continuous domains first — this makes it easy to port algorithms across different representations`],
    answer: 3,
    answerText: ``,
